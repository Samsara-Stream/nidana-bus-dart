...
- [Nidana Bus - A Reactive Data-Flow Architecture for Mobile and Web Applications](#nidana-bus---a-reactive-data-flow-architecture-for-mobile-and-web-applications)
  ...
  - [8. Cross-Cutting Concerns](#8-cross-cutting-concerns)
    - [8.1 Patterns for Cross-Cutting Concerns](#81-patterns-for-cross-cutting-concerns)
    - [8.2 Envelope Interception](#82-envelope-interception)
    - [8.3 Navigation as a Cross-Cutting Concern](#83-navigation-as-a-cross-cutting-concern)
      ...
      - [The Navigation Topology](#the-navigation-topology)
      - [The NavigationExecutor Boundary](#the-navigationexecutor-boundary)
  ...
  - [12. Emergent Architectural Properties](#12-emergent-architectural-properties)
    - [12.1 AI-Agent Compatibility](#121-ai-agent-compatibility)
    ...
    - [12.2 Cross-Platform Logic Portability](#122-cross-platform-logic-portability)
      ...
      - [Relationship to AI-Agent Compatibility](#relationship-to-ai-agent-compatibility)
  - [13. Integration With Existing UI Patterns](#13-integration-with-existing-ui-patterns)
    - [13.1 The Recommended Path: Topology Output Directly to UI](#131-the-recommended-path-topology-output-directly-to-ui)
    - [13.2 The BLoC / MVVM / MVI Interposition Problem](#132-the-bloc--mvvm--mvi-interposition-problem)
    - [13.3 When Interposition Might Be Acceptable](#133-when-interposition-might-be-acceptable)
    - [13.4 Summary](#134-summary)
  - [14. Platform Mapping](#14-platform-mapping)
    - [14.1 Reactive Primitives](#141-reactive-primitives)
    - [14.2 Lifecycle Integration](#142-lifecycle-integration)
    - [14.3 Topic Registry: Platform Syntax](#143-topic-registry-platform-syntax)
      - [Dart](#dart)
      - [Kotlin](#kotlin)
      - [Swift](#swift)
    - [14.4 Platform-Specific Topology Syntax](#144-platform-specific-topology-syntax)
    ...
    - [14.5 Platform Decision Matrix: Rx vs. Native Reactive](#145-platform-decision-matrix-rx-vs-native-reactive)
  - [15. Web and TypeScript](#15-web-and-typescript)
    - [15.1 TypeScript Core](#151-typescript-core)
    - [15.2 React Integration](#152-react-integration)
      - [Lifecycle Mapping for React](#lifecycle-mapping-for-react)
    - [15.3 Angular Integration](#153-angular-integration)
      - [Lifecycle Mapping for Angular](#lifecycle-mapping-for-angular)
    - [15.4 Vue Integration](#154-vue-integration)
      - [Lifecycle Mapping for Vue](#lifecycle-mapping-for-vue)
    - [15.5 Addressing React Community Resistance](#155-addressing-react-community-resistance)
    - [15.6 Web-Specific Considerations](#156-web-specific-considerations)
  - [16. Comparison With Existing Patterns](#16-comparison-with-existing-patterns)
  ...
  - [17. Open Questions](#17-open-questions)
    ...
    - [17.5 Persistence and Hydration](#175-persistence-and-hydration)
  - [18. FAQ for Skeptics](#18-faq-for-skeptics)
  ...
  - [Appendix C: Library Modularization and Development Phases](#appendix-c-library-modularization-and-development-phases)
    - [C.1 Decomposition Rationale](#c1-decomposition-rationale)
    - [C.2 Package Dependency Graph](#c2-package-dependency-graph)
    - [C.3 Package Descriptions](#c3-package-descriptions)
    - [C.4 Development Sequencing](#c4-development-sequencing)
    - [C.5 Dependency Rules](#c5-dependency-rules)
...
- **Typed over stringly-typed.** Topics are first-class typed reference objects, not raw strings. A `Topic<AuthState>` is a compile-time–checked, IDE-discoverable reference. The underlying string identifier is an internal implementation detail (see [§3: Topic Registry](#3-topic-registry-and-type-safety)).
...
- The **topology activation/deactivation machinery** (see [§6: Lifecycle Management](#6-lifecycle-management)).
...
```
// Pseudocode - platform-specific syntax in Section 14
topology("checkout-flow") {
...
**What the topology body may and may not do.** The builder block (the body of the `topology("...") { ... }` call, or the `declare()` method in [class-based platform syntax, §14.4](#144-platform-specific-topology-syntax)) should contain only stream wiring operations: `read`, `write`, `combine`, `map`, `on`, and other combinator calls. It should not perform I/O, access external state, or contain imperative logic that depends on runtime values. Conditional wiring based on compile-time configuration (e.g., a build-time feature flag constant) is acceptable. Reading a topic's current value synchronously to decide which streams to wire is not, because it introduces path dependence on activation order. If wiring must vary at runtime, model it as a data-driven topology that reads a configuration topic and uses reactive operators (e.g., `switchMap` on a feature flag stream) to select between pipelines.
...
Error-handling operators (`retryWhen`, `catchError`, `onErrorResumeNext`) follow the same principle. They appear inside topology pipelines to catch exceptions from effectful stream sources and convert them to typed error values on topics. They are the mechanism by which shell-boundary failures enter the topology's error-as-values model (see [§9](#9-error-handling-strategies)).
...
**DevTools foundation.** The JSON renderer produces the input for a live browser-based DevTools panel showing the running system graph, highlighted in real time as messages flow through topics. This is the natural evolution of the topology graph visualizer described in [§17.2](#172-devtools-and-observability).
...
graph TB
    subgraph BUS ["Nidana Bus (Pure Core)"]
        ...
        TOPO_NAV["Topology:<br/>Navigation Core"]
    end
    ...
    subgraph CC_NAV ["Service: Navigation"]
        NAV_SIDE["Side Effect:<br/>Platform Router"]
    end
    ...
    style CC_NAV fill:none,stroke:#059669,stroke-dasharray: 5 5,stroke-width:2px,color:#059669
    ...
    style T_ROUTE fill:#e0e7ff,stroke:#4f46e5,color:#1e293b
    ...
    style TOPO_NAV fill:#f3e8ff,stroke:#7c3aed,color:#1e293b
    ...
    style NAV_SIDE fill:#fef3c7,stroke:#d97706,color:#1e293b
```

### 8.1 Patterns for Cross-Cutting Concerns
...
| **Navigation** | Service | Reads `NavIntent` events from pages, applies guards and deep link resolution, delegates to platform router. See [§8.3](#83-navigation-as-a-cross-cutting-concern). |
| **Deep linking** | Service | Reads OS intent/URL (side effect), writes to `Topic<NavIntent>`. Subsumes into navigation. |
| **Device location** | Service | Observes OS location APIs (side effect), writes to `Topic<DeviceLocation>`. Topologies that need location data read this topic rather than querying the OS directly. |
| **OS permissions** | Service | Queries and requests OS permissions (side effect), writes to `Topic<PermissionGrant>`. Guard topologies can block intents (navigation, camera access, etc.) until the required permission is granted, using the same pattern as auth guards. |

### 8.2 Envelope Interception
...
Navigation is a side effect. The architecture's rule is: side effects happen at the shell boundary. Yet in conventional mobile and web applications, navigation calls are scattered across every page and module that needs to move the user somewhere. A page calls `Navigator.push(...)` or `router.navigate(...)` directly, performing the side effect inline.
...
Pages publish navigation intents to an `EventTopic<NavIntent>`. A dedicated `NavigationService` at the upper shell subscribes to that topic and performs the actual platform navigation calls. The `NavIntent` sealed type captures the vocabulary:

```
// Platform-agnostic navigation intent ADT
...
```
// Pseudocode
topology("navigation-core", scope = Scope.APPLICATION_EAGER) {
...
The pattern is opt-in. The `nidana-navigation` package (see [Appendix C](#appendix-c-library-modularization-and-development-phases)) is an optional dependency. Apps that do not need centralized navigation simply omit it.
...
4. **Transformers must not throw.** Throwing an exception inside a `map` or `combine` operator will terminate the topology's subscription (see [§2.3](#23-bus-runtime)). Developers must catch expected exceptions at the shell boundary or within the transformer and emit them as values (e.g., using `Result<T, E>`). Performing side-effects inside operators is at the developer's own risk and is strongly advised against.
...
| **Unhandled exceptions in business logic** | Transformers are pure functions. When errors are modeled as values (`Result<T, E>`) per [§9](#9-error-handling-strategies), there is no exception to throw. The failure is data on a topic, not a stack-unwinding crash. The architecture *enables* exception-free business logic but does not *enforce* it; a developer can still throw, index out of bounds, or unwrap null inside a transformer. Such exceptions are confined to the topology's error boundary: the underlying reactive framework terminates that topology's subscription, not the backing subject itself (see [§2.3](#23-bus-runtime)). Other topologies subscribed to the same topic are unaffected. | `NullPointerException` deep in a ViewModel, uncaught `Future` errors |
...
The architecture has meaningful connections to structures studied in category theory. These connections are not merely decorative. They explain *why* the composition model works and what invariants it preserves. The properties described below hold under the assumption that topologies use pure transformers and that stream sources behave deterministically. Topologies that include effectful stream sources (see the effectful stream boundary note in [§5.3](#53-reactive-combinators)) or time-dependent operators (see [§11.2](#112-determinism)) fall outside the pure subset and the categorical guarantees apply only to the pure portions of the pipeline.
...
## 12. Emergent Architectural Properties

The pure-core/imperative-shell separation produces benefits beyond the immediate goals of decoupling and testability. This section documents properties that emerge structurally from the architecture's constraints, particularly from the requirement that transformers are pure functions and topologies are declarative data structures. These are not features that must be built; they are consequences that can be harvested.

### 12.1 AI-Agent Compatibility
...
### 12.2 Cross-Platform Logic Portability

The pure-core/imperative-shell separation produces a second structural property: **the entire logic layer is portable across platforms by construction.**

A transformer is a pure function with signature `(MessageEnvelope<S>) → MessageEnvelope<D>`. It imports no platform framework, touches no lifecycle API, performs no I/O. A topology definition is a declarative data structure describing which topics connect through which transformers. Neither layer contains anything platform-specific. The only platform-dependent code lives in the imperative shells (runtime adapters, services, UI), which are explicitly excluded from the pure core.

This means the architecture naturally supports a **single source of truth for business logic** across Dart/Flutter, Kotlin/Android, Swift/iOS, and TypeScript/Web, with two viable strategies for realizing it.

#### Strategy 1: Portable DSL

Define both transformers and topology wiring in a platform-neutral DSL. A code generator then emits platform-specific implementations.
...
transform: computePricing
  # Expression language for simple transforms
  rules:
    - apply_discount(items, tier)
    - apply_tax(discounted, region)
```

The DSL approach works best when transformers remain simple (mapping, filtering, combining, reshaping data). The topology declarations are already declarative and map directly to a DSL grammar. The constraint is that transformers must stay within the expression language's capabilities; anything that exceeds them signals logic that may belong in a service (the imperative shell) rather than the pure core.

#### Strategy 2: Single-Language SSOT with Code Generation

Write transformers and topology definitions in one language, then generate equivalent code for other platforms. Kotlin is a natural candidate here because Kotlin Multiplatform (KMP) already targets JVM, native iOS, and JavaScript. Dart is equally viable as the primary implementation language if Flutter is the lead platform.

The key requirement is the same in either case: the transformer function must be expressible as a pure data transformation with no platform-specific imports. If this constraint holds, mechanical translation is straightforward because there is nothing platform-specific to adapt.

```
┌─────────────────────────────────────────────────┐
│           SSOT (Dart or Kotlin)                 │
│                                                 │
│   Transformers:  (A) → B  (pure functions)      │
│   Topologies:    declarative wiring definitions │
└──────────────┬──────────────┬───────────────────┘
               │              │
         ┌─────▼─────┐  ┌─────▼──────┐
         │  codegen  │  │  codegen   │
        ...
        (Combine, UIKit)  (RxJS, React)
```

#### The Portability Constraint as Architectural Pressure

This property is not merely a nice-to-have. It functions as a **design-time enforcement mechanism**. The moment a developer attempts to introduce a network call, database access, or platform API invocation into a transformer, the portability guarantee breaks visibly. The function can no longer be generated for other platforms. This creates constructive pressure to keep the pure core genuinely pure, pushing effects outward to services where they belong.

In practice, this means:

- **Transformer purity is verifiable by tooling.** A linter or code generator can reject any transformer that imports platform-specific packages.
- **The boundary between portable and non-portable code is explicit.** It aligns exactly with the pure-core/imperative-shell boundary, reinforcing the architecture's central invariant rather than introducing a new one.
- **Testing portability comes for free.** If transformers are pure and platform-independent, the same test cases (input envelope, expected output envelope) are valid on every platform. Only the test harness differs.

#### Scope and Limitations

Not all application logic belongs in the portable layer. Effects (network, persistence, sensors, navigation execution) are inherently platform-specific and remain in services and runtime adapters. The portable layer covers the **coordination and transformation logic** that sits between those boundaries, which, in a well-structured Nidana Bus application, constitutes the majority of business rules.

The DSL approach introduces a language boundary that adds tooling cost. The SSOT approach ties the canonical implementation to one language's semantics. Both are valid trade-offs; the choice depends on team composition and platform priority. What the architecture guarantees is that either approach is structurally sound, because the pure core was designed to be platform-independent from the start.

#### Relationship to AI-Agent Compatibility

Sections 12.1 and 12.2 are mutually reinforcing. AI agents benefit from bounded, pure-function tasks (12.1). Cross-platform code generation requires bounded, pure-function inputs (12.2). The same architectural constraint that makes a transformer easy for an AI to reason about also makes it mechanically translatable across languages. An AI agent could, in principle, serve as the code generator itself: given a pure transformer in Dart and target type definitions in Swift, generating the Swift equivalent is a well-bounded translation task with a verifiable output.


## 13. Integration With Existing UI Patterns

### 13.1 The Recommended Path: Topology Output Directly to UI
...
### 13.2 The BLoC / MVVM / MVI Interposition Problem
...
### 13.3 When Interposition Might Be Acceptable
...
The architecture is platform-agnostic in concept. This section maps each concept to mobile platform-specific primitives. For web/TypeScript mapping (React, Angular, Vue), see [§15: Web and TypeScript](#15-web-and-typescript).

### 14.1 Reactive Primitives
...
### 14.3 Topic Registry: Platform Syntax
...
```
### 14.4 Platform-Specific Topology Syntax
...
### 14.5 Platform Decision Matrix: Rx vs. Native Reactive
...
### 15.5 Addressing React Community Resistance
...
### 15.6 Web-Specific Considerations
...
## 16. Comparison With Existing Patterns
...
### 16.1 Complementary, Not Competing
...
- **Topology graph visualizer.** Since every topology is pure data with declared reads/writes, `TopologyDefinition.toGraph()` and `Bus.toGraph()` provide the foundation for this without any special instrumentation. See [§5.5](#55-topology-as-self-documenting-data) for the full design. The open question is which renderer targets to ship as first-class library support vs. leave to integrators.
...
**What the current design already provides.** A `TopologyDefinition` is pure inert data. The `TopologyGraph` IR introduced in [§5.5](#55-topology-as-self-documenting-data) for diagram rendering is structurally identical to what a topology deserializer would consume: typed nodes and directed edges, with topic names as string keys. The bus-lazy topic creation model ([§2.5](#25-topic-initialization)) means dynamically named topics created at runtime are handled identically to statically declared ones. These are not accidental properties; the architecture does not need to change to support this path.
...
Nidana Bus is not a replacement for screen-level state management (see [§13](#13-integration-with-existing-ui-patterns)). It is an *inter-module coordination layer* that sits above individual screens. You can adopt it incrementally: start with one cross-cutting concern (e.g., connectivity monitoring) as a service with a topology, and expand as the benefits become clear.
...
The adoption path is incremental (see [§15.5](#155-addressing-react-community-resistance)). Start with one service topology (e.g., connectivity). Expose it via a `useTopic` hook. The consuming component looks like any other React code. Existing Zustand stores continue to work unchanged.
...
| **NavIntent** | A sealed ADT representing a navigation intent (GoTo, Back, Replace, DeepLink, etc.) published by pages and consumed by the NavigationService. See [§8.3](#83-navigation-as-a-cross-cutting-concern). |
| **NavigationExecutor** | The shell-boundary adapter that translates resolved `NavIntent` values into platform-specific navigation calls (GoRouter, Navigation Component, NavigationStack, React Router). |
| **Route** | A typed reference to a navigation destination, analogous to `Topic<T>` for data channels. Provides compile-time safety and IDE discoverability for navigation targets. |
...
graph TB
    subgraph SERVICES ["Services (Upper Imperative Shell)"]
        ...
        SVC_NAV["Navigation Service<br/><i>side effect: platform router</i>"]
    end
    ...
    style SVC_NAV fill:#fef3c7,stroke:#d97706,color:#1e293b
    style TOPO_AUTH fill:#f3e8ff,stroke:#7c3aed,color:#1e293b
    style TOPO_PAY fill:#f3e8ff,stroke:#7c3aed,color:#1e293b
    style TOPO_CACHE fill:#f3e8ff,stroke:#7c3aed,color:#1e293b
    style TOPO_CHECKOUT fill:#f3e8ff,stroke:#7c3aed,color:#1e293b
    ...
    style T9 fill:#e0e7ff,stroke:#4f46e5,color:#1e293b
...
## Appendix C: Library Modularization and Development Phases

The architecture described in this document is too large for a single library. The concepts decompose naturally into layers with clean dependency boundaries. This appendix defines the package structure, dependency rules, and recommended development sequencing.

### C.1 Decomposition Rationale

The decomposition follows two principles:

1. **Each package has a single reason to change.** The bus core changes when the topic/publish/subscribe model changes. The topology layer changes when the builder DSL or graph introspection changes. The runtime adapters change when platform lifecycle APIs change. These are independent change vectors.

2. **Dependencies flow downward.** Higher-level packages depend on lower-level packages, never the reverse. The bus core knows nothing about topologies. The topology layer knows nothing about platform lifecycles. The runtime adapters compose the lower layers with platform-specific bindings.

An application can adopt only the layers it needs. A project that only wants typed topics and publish/subscribe (without topologies) takes only `nidana-core`. A project that wants topologies but handles its own lifecycle management takes `nidana-core` + `nidana-topology`. The full stack adds the runtime adapter for the target platform.

### C.2 Package Dependency Graph

```mermaid
graph TB
    CORE["nidana-core<br/><i>bus, topics, envelopes, registry</i>"]
    TOPO["nidana-topology<br/><i>builder API, definition, graph introspection</i>"]
    NAV["nidana-navigation<br/><i>(optional) NavIntent ADT, navigation topics,<br/>guard topology, abstract executor</i>"]
    RT_FL["nidana-runtime-flutter<br/><i>lifecycle binding,<br/>NavigationExecutor (GoRouter)</i>"]
    RT_CMP["nidana-runtime-compose<br/><i>lifecycle binding,<br/>NavigationExecutor (Nav Component)</i>"]
    RT_SUI["nidana-runtime-swiftui<br/><i>lifecycle binding,<br/>NavigationExecutor (NavigationStack)</i>"]
    RT_WEB["nidana-runtime-web<br/><i>lifecycle binding (React/Angular/Vue),<br/>NavigationExecutor (React Router, etc.)</i>"]
    TEST["nidana-test<br/><i>TestBus, assertion helpers,<br/>virtual scheduler</i>"]
    DEV["nidana-devtools<br/><i>graph renderers, topology diff,<br/>envelope inspector</i>"]
    BLOC["nidana-bloc-bridge<br/><i>(optional) BLoC adapter for<br/>gradual migration</i>"]
    GEN["nidana-codegen<br/><i>(optional) topic registry<br/>generation from schema</i>"]
    ...
    style RT_FL fill:#fef3c7,stroke:#d97706,color:#1e293b
    style RT_CMP fill:#fef3c7,stroke:#d97706,color:#1e293b
    style RT_SUI fill:#fef3c7,stroke:#d97706,color:#1e293b
    style RT_WEB fill:#fef3c7,stroke:#d97706,color:#1e293b
...
### C.3 Package Descriptions

**nidana-core** is the foundational layer. It contains `Topic<T>` (all three variants), the topic registry, `MessageEnvelope`, publish/subscribe mechanics, and the bus singleton. Its only external dependency is the platform's reactive library (RxDart for Dart, RxJS for TypeScript, etc.). This package is usable standalone as a typed event bus with envelope tracing, without topologies or lifecycle management. Everything else builds on top of it.

**nidana-topology** adds the declarative topology layer: the `TopologyBuilder` DSL, `TopologyDefinition` as an inert data structure, the `TopologyGraph` intermediate representation and `toGraph()` introspection, scope declarations, and the activation/deactivation contract. It depends on nidana-core. It is still platform-agnostic and framework-agnostic: it defines the topology abstraction but does not bind it to any platform's lifecycle system. A topology can be activated and deactivated manually using the handle-based API described in [§6.3](#63-approach-b-owner-managed-lifecycle) without any runtime adapter.

**nidana-navigation** *(optional)* adds the stream-based navigation model described in [§8.3](#83-navigation-as-a-cross-cutting-concern). It contains the `NavIntent` sealed type, `Route` typed references, `NavigationTopics` (intent, currentRoute, history), the navigation guard topology, and the abstract `NavigationExecutor` contract. It depends on nidana-topology (the guard topology uses the builder API). It is platform-agnostic: the concrete executor implementation lives in the platform runtime package. Apps that do not need centralized navigation simply omit this dependency.

**nidana-runtime-flutter** (and the equivalent `-compose`, `-swiftui`, `-web` packages) bind the topology lifecycle to the platform's lifecycle events. For Flutter, this means `State.initState`/`dispose` for page scope, `RouteObserver` for module scope, and `WidgetsBindingObserver` for application scope. It also provides the concrete `NavigationExecutor` for the platform's routing framework (GoRouter adapter, Navigator 2.0 adapter, etc.), activated only if `nidana-navigation` is in the dependency graph. The navigation executor dependency is optional: if the app does not depend on `nidana-navigation`, the runtime package simply does lifecycle binding for topologies.

**nidana-test** provides the `TestBus`, synthetic topic publishers, assertion helpers for topology output verification, and virtual time scheduler injection for deterministic testing of time-dependent operators. It depends on nidana-topology (to activate topologies on the test bus). It does not depend on any platform runtime, making it usable in pure unit tests without a Flutter/Android/iOS test harness.

**nidana-devtools** provides the graph renderers (Mermaid, Graphviz, JSON), topology diff tooling, the envelope inspector, and the build-time topology catalog generator described in [§5.5](#55-topology-as-self-documenting-data) and [§17.2](#172-devtools-and-observability). It depends on nidana-topology for `TopologyGraph` access.

**nidana-bloc-bridge** *(optional)* provides an adapter for existing Flutter BLoC codebases. It allows a BLoC to subscribe to a topic and republish topic emissions as BLoC state, enabling gradual migration from BLoC to topology-driven UI as described in [§13.3](#133-when-interposition-might-be-acceptable). It depends only on nidana-core (for topic subscription) and `flutter_bloc`, not on nidana-topology. A BLoC can subscribe to any topic regardless of which topology writes to it.

**nidana-codegen** *(optional)* generates topic registry classes from a schema file (e.g., `topics.yaml`) as described in [§3.5](#35-code-generation-approach-optional). It depends only on nidana-core (for the `Topic<T>` types it generates). It runs as a build-time tool, not a runtime dependency.

### C.4 Development Sequencing

The packages should be developed in dependency order. Each phase produces a usable, shippable artifact before the next phase begins.

| Phase | Package | Deliverable | Usable Standalone? |
|---|---|---|---|
| 1 | **nidana-core** | Typed event bus with topics, envelopes, publish/subscribe | Yes. A typed event bus is useful on its own. |
| 2 | **nidana-topology** | Declarative topology builder, graph introspection, manual activation | Yes, with manual lifecycle management. |
| 3 | **nidana-navigation** *(optional)* | NavIntent ADT, navigation topics, guard topology, abstract executor | Yes, with manual executor wiring. |
| 4 | **nidana-runtime-flutter** (primary target) | Automatic lifecycle binding, concrete NavigationExecutor | Full framework integration for Flutter. |
| 5 | **nidana-test** | TestBus, assertion helpers, virtual scheduler | Testing infrastructure for phases 1-4. |
| 6 | **nidana-devtools** | Graph renderers, topology diff, catalog generator | Observability and documentation tooling. |
| 7 | **nidana-bloc-bridge** *(optional)* | BLoC adapter for gradual migration | Migration bridge for existing BLoC apps. |
| 8 | **nidana-codegen** *(optional)* | Schema-driven topic registry generator | Build-time tooling for large teams. |
| 9+ | **nidana-runtime-compose**, **-swiftui**, **-web** | Platform runtime adapters for other targets | Full framework integration per platform. |

**Phase 1 is the critical milestone.** A working, tested, published `nidana-core` validates the foundational abstractions (topic variants, envelope propagation, type enforcement) before any topology complexity is layered on. If the core model is wrong, everything above it is wrong. Ship it first, use it in a real project, and iterate before building higher.

**Phase 5 (testing) is listed after the runtime,** but in practice, testing utilities should be developed alongside each phase. The sequencing reflects *publication* order, not development order. The `TestBus` concept should exist from phase 1 as an internal tool, formalized into its own package once the topology API stabilizes.

### C.5 Dependency Rules

These rules ensure the package graph remains clean as the library evolves:

1. **No upward dependencies.** nidana-core must never depend on nidana-topology. nidana-topology must never depend on any runtime adapter. A lower layer cannot know about a higher layer.

2. **No cross-platform dependencies.** nidana-runtime-flutter must never depend on nidana-runtime-compose. Platform adapters are siblings, not dependencies of each other.

3. **Optional dependencies are explicit.** nidana-runtime-flutter's dependency on nidana-navigation is optional. If the navigation package is not in the dependency graph, the runtime adapter must still compile and function. This means the runtime adapter must use conditional imports or plugin registration, not hard imports of navigation types.

4. **The reactive library is a hard dependency of core.** nidana-core takes RxDart (for Dart), RxJS (for TypeScript), etc. as a direct dependency. The topology layer and all higher packages inherit it transitively. There is no abstraction layer over reactive primitives. The reactive library is the execution engine, and abstracting it would add complexity without benefit: the topology DSL already hides Rx from consumers ([§14.5](#145-platform-decision-matrix-rx-vs-native-reactive)).

5. **Platform framework dependencies are confined to runtime packages.** Only nidana-runtime-flutter depends on Flutter. Only nidana-bloc-bridge depends on flutter_bloc. The core, topology, navigation, test, and devtools packages are pure Dart (or pure TypeScript, etc.) with no framework dependency.
